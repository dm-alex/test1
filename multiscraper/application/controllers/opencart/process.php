...
class Process extends CI_Controller {
	...
	private function _processCategory($task , $settings){
	    ...
	    $inv_mode = false;
	    // INVISIBLE MODE JUST THROUGH THE PARSERS
		...
		if(function_exists('mspro_' . $market['name']  . '_category_getUrl')){
	...
	private function _processProduct($task , $settings){
		...
		//echo $task['url'];
		$html = false;
		if(function_exists('mspro_' . $market['name']  . '_getUrl')){
		   $html = call_user_func('mspro_' . $market['name']  . '_getUrl' , $task['url']);
		}else{
		  $html = getUrl($task['url'] , false , $inv_mode);
		}
		//echo 'HTML:' . var_dump($html);exit;
			...
			if(isset($option['name']) && isset($option['values']) && is_array($option['values']) && count($option['values']) > 0 ){
				...
				foreach($option['values'] as $option_value){
					if(isset($option_value['name']) && isset($option_value['price'])){
						$OPTION_VALUE = array();
						$OPTION_VALUE['option_value_id'] = $this->_getOptionValue($option_value['name'] , $OPTION['option_id'] , isset($option_value['image'])?$option_value['image']:"" , $OPTION['type']);
	...
	function _getOptionValue($valueName , $option_id , $image = '' , $type){
		$res = cms_getOptionValue($valueName, $option_id , $type , $this->translit_name);
		if(!$res){
		    if($type == "image" && strlen(trim($image)) > 1){
