...
function cms_getAttribute($name , $groupID){
    $ci =& get_instance();
    $ci->load->database();
    $query = 'SELECT ads.attribute_id FROM ' . DB_PREFIX. 'attribute_description ads RIGHT JOIN ' . DB_PREFIX. 'attribute a ON (ads.attribute_id = a.attribute_id) WHERE ads.name = "' . trim($name) . '" AND a.attribute_group_id = ' . trim($groupID) . '';
    //echo $query;exit;
    $res = $ci->db->query($query)->result();
    if (count($res) > 0){
        $res = $res[0];
